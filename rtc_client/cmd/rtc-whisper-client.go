...
func riaSaysHello(ae *rtc_client.AudioEngine, rtc *rtc_client.RTCConnection, client_id string) int {
	...
	for {
		...
		if resp.StatusCode == http.StatusOK {
			...
			data := make([]float32, 15825) // prepending silence. this was 4800 (previously 38050, idk why we increased) and now increasing by 500ms, the duration of the pick up sound
