module github.com/infinityp913/rtc-go-server/rtc_client

go 1.20

require (
	github.com/GRVYDEV/S.A.T.U.R.D.A.Y/log v0.0.0-20230726165431-f1d355a1a072
	github.com/GRVYDEV/S.A.T.U.R.D.A.Y/util v0.0.0-20230729192628-8d783e5be014
	github.com/gorilla/websocket v1.5.0
	github.com/infinityp913/rtc-go-server/stt/backends/whisper.cpp v0.0.0-20231003180621-4e22dde16b75
	github.com/infinityp913/rtc-go-server/stt/engine v0.0.0-20231003170449-571d9420218d
	github.com/pion/ion-sfu v1.11.0
	github.com/pion/rtp v1.8.2
	github.com/pion/webrtc/v3 v3.1.25
	github.com/pion/webrtc/v4 v4.0.0-beta.6
	gopkg.in/hraban/opus.v2 v2.0.0-20220302220929-eeacdbcb92d0
)

require (
	github.com/ggerganov/whisper.cpp/bindings/go v0.0.0-20230716200034-399846572185 // indirect
	github.com/go-logr/logr v1.2.4 // indirect
	github.com/go-logr/zerologr v1.2.3 // indirect
	github.com/google/uuid v1.3.1 // indirect
	github.com/mattn/go-colorable v0.1.12 // indirect
	github.com/mattn/go-isatty v0.0.14 // indirect
	github.com/pion/datachannel v1.5.5 // indirect
	github.com/pion/dtls/v2 v2.2.7 // indirect
	github.com/pion/ice/v2 v2.2.2 // indirect
	github.com/pion/interceptor v0.1.24 // indirect
	github.com/pion/logging v0.2.2 // indirect
	github.com/pion/mdns v0.0.8 // indirect
	github.com/pion/randutil v0.1.0 // indirect
	github.com/pion/rtcp v1.2.10 // indirect
	github.com/pion/sctp v1.8.9 // indirect
	github.com/pion/sdp/v3 v3.0.6 // indirect
	github.com/pion/srtp/v2 v2.0.5 // indirect
	github.com/pion/stun v0.3.5 // indirect
	github.com/pion/transport v0.14.1 // indirect
	github.com/pion/transport/v2 v2.2.4 // indirect
	github.com/pion/turn/v2 v2.0.8 // indirect
	github.com/rs/zerolog v1.29.1 // indirect
	golang.org/x/crypto v0.14.0 // indirect
	golang.org/x/exp v0.0.0-20230522175609-2e198f4a06a1 // indirect
	golang.org/x/net v0.16.0 // indirect
	golang.org/x/sys v0.13.0 // indirect
)
